...
import { ALL_ROLES, ALL_USERS, NEW_PASSWORD, NEW_USER, URLALLROLES, URLALLUSER, URLCREATEUSER, URLNEWPASSWORD } from "./ActionsTypes";
...
export function newPassword({token, id}){
  return async (dispatch) => {
    try{
      const response = await axios.put(`${URLNEWPASSWORD}/${id}`,{
        headers: { Authorization: `Bearer ${token}` },
      });
